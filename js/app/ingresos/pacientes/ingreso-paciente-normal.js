...
function camposVacios(){
	var inputs = $("#form-pacientes :input");
	var vacios = true;
	for (var i = 0; i < inputs.length; i++) 
	{
		switch(inputs[i].type)
		{
			case "text": inputs[i].value !== "" ? vacios = false : "";
				break;
			case "number": inputs[i].value !== "" ? vacios = false : "";
				break;
			case "email": inputs[i].value !== "" ? vacios = false : "";
				break;
			case "date": inputs[i].value !== "" ? vacios = false : "";
				break;
		}		
	}
	return vacios;
...
function ObtenerNumPaciente(){
	$.ajax({
		type: "GET",
		contentType: "application/json",
		url: "api/pacientes/ultimo",
		dataType: "json",
		headers: {"Authorization": $.cookie("ApiKey")},
		success: function(response){
			$("#num-paciente").text(response.data);
		},
		error: function(error){
			$.bigBox({
				title : "Error",
				content : "No se puede obtener el numero del siguiente paciente.",
				color : "#C46A69",
				timeout: 5000,
				icon : "fa fa-warning shake animated"
			});
